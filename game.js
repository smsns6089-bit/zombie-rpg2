Js
// Project Game Maker: Zombie RPG FPS (Raycast) - FULL REWRITE v3
// GitHub Pages friendly. No libraries.
//
// FIXES/ADDS:
// - FIXED: mouse look up/down normal (not inverted)
...
function userGesture() {
  ...
  if (groanAudio) {
    // "unlock" audio on iOS by playing silently once
    try {
...
function sfxZombieGroan(dToPlayer = 6) {
  ...
  if (vol < 0.05) return;
  try {
    // clone play by resetting currentTime
...
addEventListener("mousemove", (e) => {
  ...
  // FIX: normal FPS pitch (mouse up = look up)
...
function rollArmorPiece() {
  // rarity chance increases with wave
  const w = game.wave;
  const r = Math.random();
  let ri = 0;
  const bump = clamp((w - 1) / 14, 0, 1); // later waves higher rarity
  ...
  const prettySlot = slot === "helmet" ? "Helmet" :
                     slot === "chest" ? "Chest" :
...
// armor store rotates each time you open shop
...
function renderShop() {
  ...
  // price scales with rarity and wave
  ...
  ui.shopList.appendChild(shopButton({
    ...
    onClick: () => {
      ...
      equipArmor(ar);
      shopArmorRoll = rollArmorPiece(); // reroll after purchase
      saveGame();
      renderShop();
    }
  }));
...
function dropMats(x, y) {
  // scrap more common than essence
  const s = Math.random();
...
function render(dt) {
  ...
  for (const m of mines) sprites.push({ kind: "mine", ...m, d: dist(player.x, player.y, m.x, m.y) });
  ...
  for (const s of sprites) {
    ...
    // drops
    if (s.kind === "drop") {
      if (s.kind === "drop" && s.kind !== "drop") continue;
    ...
    if (s.kind === "drop") {
      ...
      // actual drop kind is inside s.kind? we stored as "drop" but original kind stored in s.kind? (we spread ...d)
      // so s.kind is "drop", but s.kind data contains 'kind' from drops object:
      const dk = s.kind === "drop" ? s.kind : s.kind;
      const real = s.kind === "drop" ? s.kind : s.kind;
      // We have the real kind in s.kind? No: spread overwrote, so use s.kind from drop object: s.kind is "cash"/"scrap"/"ess"/"armor"
    }
  }

  // Re-draw drops properly (clean pass, no weird overwrite)
  // (We do a second simple pass only for drops to avoid spread overwrite confusion.)
  ...
  if (nearShopKiosk() && game.mode === "play") {
    ...
    ctx.fillText(`Medkits ${player.medkits} | Mines ${player.mineCount} | Scrap ${player.scrap} | Essence ${player.essence} | Armor ${getTotalArmor()}`, w * 0.315, h * 0.675);
  }
}
...
function tick(now) {
  ...
  // build pathfinding flow field every 0.25s
  game.flowTimer -= dt;
  if (game.flowTimer <= 0) {
    game.flowTimer = 0.25;
  ...
  for (let i = drops.length - 1; i >= 0; i--) {
    ...
    if (dist(player.x, player.y, d.x, d.y) < 0.55) {
      if (d.kind === "cash") {
        ...
        // auto-equip if slot empty, else just equip if better
...
setHint("Click to play. Find the green SHOP kiosk. Press Q. Sprint = Shift. Medkit = H. Mine = G.");
requestAnimationFrame(tick);
