...
(() => {
 ...
 function drawWeaponViewmodel(w, dt) {
  ...
  // -------- Viewmodel placement (more centered + lower like FPS) --------
  const baseX = W * 0.62;
  const baseY = H * 0.86;

  // -------- Recoil behavior (mostly back/up, not sideways) --------
  const kick = game.recoil * 60;
  const mx = -kick * 0.12;   // tiny sideways
  const my =  kick * 0.68;   // mostly up/back

  // -------- Subtle idle sway (reduced so it feels steadier) --------
  const t = performance.now() / 1000;
  const swayX = Math.sin(t * 1.8) * 1.4;
  const swayY = Math.cos(t * 2.1) * 1.1;
  ...
  const forwardAngle = -0.68;
  ctx.rotate(forwardAngle + game.recoil * 0.05);
  ...
  if (player.usingKnife) {
    ...
    fillPart(-20*scale, 56*scale, 290*
